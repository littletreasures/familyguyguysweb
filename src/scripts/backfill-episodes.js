...
if (!supabaseUrl) {
  console.error('CRITICAL SECURITY ERROR: SUPABASE_URL is not set.');
  process.exit(1);
}

if (!supabaseKey) {
  console.error(
    'CRITICAL SECURITY ERROR: SUPABASE_SECRET_KEY (or SUPABASE_SERVICE_KEY) is required for database write operations.'
  );
  console.error('Do not use public anon keys for write scripts.');
  process.exit(1);
}
...
const episodesToUpdate = [
  {
    ...
    writers: ['Seth MacFarlane', 'David Zuckerman', 'Mike Henry'],
  ...
  },
  {
    ...
    imdb_rating: '7.5',
    summary:
      "Peter teaches Meg to drive and immediately turns the town's satellite dish into scrap metal. That's the real episode. What we actually talk about: an AI that tried to summarize the season for us and just started inventing cults out of thin air, Jedi semen retention, Al Pacino (briefly), and so many stories about convertibles it'll make your head spin.",
    cast: ['Seth MacFarlane', 'Alex Borstein', 'Seth Green'],
    writers: ['Seth MacFarlane', 'David Zuckerman', 'Chris Sheridan'],
  ...
  },
  {
    ...
    cast: ['Seth MacFarlane', 'Alex Borstein', 'Seth Green'],
    writers: ['Seth MacFarlane', 'David Zuckerman', 'Danny Smith'],
...
async function run() {
  ...
  for (const ep of episodesToUpdate) {
    ...
    if (error) {
      ...
      console.warn(
        `Warning: No rows updated for ${ep.id}. This occurs if write permission is denied.`
      );
    } else {
